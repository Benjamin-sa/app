...
- **What Services DON'T Do:**
  ...
  - **No HTTP Logic:** Services do not know about `req` or `res` objects. They receive plain data from the controllers and return data or throw errors.
...
Our error handling strategy is systematic and designed for clarity.

1.  **Service-Level Errors:** A service performs validation. If it fails, it throws an error with a specific prefix.

    - `throw new Error("VALIDATION_ERROR: ...");`
    - `throw new Error("NOT_FOUND_ERROR: ...");`
    - `throw new Error("SERVICE_ERROR: ...");`

2.  **Controller-Level Catching:** The controller, which called the service, catches this error in a `try...catch` block.
...
3.  **Client Response:** The user receives a consistent JSON error object, which the frontend can then use to display an appropriate message.
