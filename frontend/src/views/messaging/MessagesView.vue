<template>
    <div class="max-w-6xl mx-auto px-4 py-8" :style="{ paddingTop: `${navbarStore.effectiveNavbarHeight + 32}px` }">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden h-[600px] flex">
            ...
            <div class="w-1/3 border-r border-gray-200 dark:border-gray-700">
                ...
                <ConversationList v-else :conversations="conversations" @select-conversation="selectConversation" />
            </div>
...
const loadConversations = async () => {
    ...
    if (result) {
        ...
        } else if (startConversation && result.conversations.length === 0) {
            // If no conversations exist and we want to start one, trigger the creation
...
const createNewConversation = async (receiverId) => {
    ...
    if (result) {
        ...
        selectedConversation.value = conversations.value.find(c => c.id === result.id);
    }
};
